...
export const ChatWsRouteLive = HttpLayerRouter.use((router) =>
  Effect.gen(function* () {
    ...
    const handleFrame =
      (user: User, send: (event: ServerEvent) => Effect.Effect<void>) =>
      (raw: string | Uint8Array) => {
        ...
        return decodeClientEvent(text).pipe(
          ...
          Effect.catchAll((error) =>
            send(new EventRejected({ reason: error.message })),
          ),
        );
      };
    ...
    const connect = (user: User) =>
      Effect.gen(function* () {
        ...
        const send = (event: ServerEvent) =>
          encodeServerEvent(event).pipe(Effect.flatMap(write), Effect.ignore);
